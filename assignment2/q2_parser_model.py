...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        embedded = tf.constant(self.pretrained_embeddings)  # (batch_size, embed_size)
        embeddings = tf.nn.embedding_lookup(embedded, self.input_placeholder)  # (batch_size, n_features, embed_size)
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier([self.config.n_features * self.config.embed_size, self.config.hidden_size]))
        ...
        U = tf.Variable(xavier([self.config.hidden_size, self.config.n_classes]))
    ...
    def add_loss_op(self, pred, reg):
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)) + \
               tf.reduce_mean(self.config.l2_reg * reg)
        return loss
    ...
    def fit(self, sess, saver, parser, train_examples, dev_set):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            if dev_UAS > best_dev_UAS:
                ...
                es_flag += 1
                if es_flag == 5:
                    print ("Early-stop condition has benn triggered !")
                    break
            print ()
...
'''
We set an early-stopping rounds = 5, which means, if the dev UAS doesn't increase for 5 rounds, then the training will stop.
The lowest train loss = 0.0753, and the test UAS = 88.02; the best dev UAS = 87.73  
'''