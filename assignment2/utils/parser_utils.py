...
class Parser(object):
    ...
    def __init__(self, dataset):
        ...
        deprel = [self.root_label] + list(set([w for ex in dataset
                                               for w in ex['label']
                                               if w != self.root_label])) # deprel contaions a list of all terms (without dublication) existed in 'label'
    ...
    def vectorize(self, examples):
        # for each element of vec_examples, there are ids of words / POSs / heads / labels in each corresponding ex.
        vec_examples = []
        for ex in examples:
    ...
    def extract_features(self, stack, buf, arcs, ex):
        ...
        # get all lc or rc with head = k, and sort in ascend order
        def get_lc(k):
        ...
        p_features = []
        l_features = []
        features = [self.NULL] * (3 - len(stack)) + [ex['word'][x] for x in stack[-3:]]  # last 3 in stack
        features += [ex['word'][x] for x in buf[:3]] + [self.NULL] * (3 - len(buf))  # use [self.NULL] * (3 - len(buf)) to make sure 3 elements
        if self.use_pos:
        ...
        for i in range(2):
            if i < len(stack):
                ...
                lc = get_lc(k)
                rc = get_rc(k)
                llc = get_lc(lc[0]) if len(lc) > 0 else []  # lc of left-most term
                rrc = get_rc(rc[0]) if len(rc) > 0 else []

                features.append(ex['word'][lc[0]] if len(lc) > 0 else self.NULL)  # self.NULL = len(tok2id)
    ...
    def get_oracle(self, stack, buf, ex):
        ...
        i0 = stack[-1]  # stack contains numbers, indicating the index of word in that position of stack
        i1 = stack[-2]
        h0 = ex['head'][i0]  # elements in 'head' are numbers
        h1 = ex['head'][i1]
        l0 = ex['label'][i0]
        l1 = ex['label'][i1]

        # return one of the member in trans
        if self.unlabeled:
            if (i1 > 0) and (h1 == i0):
                return 0  # 'L'
            elif (i1 >= 0) and (h0 == i1) and \
                 (not any([x for x in buf if ex['head'][x] == i0])):  # for any x in buffer, x's head != i0, or x == 0
                return 1  # 'R'
            else:
                return None if len(buf) == 0 else 2  # 'S'
        else:
            if (i1 > 0) and (h1 == i0):
    ...
    def legal_labels(self, stack, buf):
        # if legal, append 1; else, append 0
    ...
    def parse(self, dataset, eval_batch_size=5000):
        ...
        for i, example in enumerate(dataset):
            ...
            sentence_id_to_idx[id(sentence)] = i  # id() returns memory
...
def read_conll(in_file, lowercase=False, max_example=None):
    ...
    with open(in_file) as f:
        ...
        for line in f.readlines():
            sp = line.strip().split('\t') # str.strip('c'): remove 'c' from the beginning of str; strip(): remove space
            if len(sp) == 10:
                if '-' not in sp[0]:
