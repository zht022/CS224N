...
# We will use sklearn here because it will run faster than implementing ourselves.
# However, for other parts of this assignment you must implement the functions yourself!
...
def getArguments():
    ...
    # define exclusive group: illegal to exist both two args
    group = parser.add_mutually_exclusive_group(required=True)
    # now set acceptable arguments
    # dest="a": you can access argument by args.a
    # help: introduce argument command
    # action="store_true": no need to receive param
...
def getRegularizationValues():
    ...
    """
    return np.sort(np.logspace(-4, 2, num=100, base=10))
...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "w") as f:
        ...
        for i in xrange(len(dataset)):
            print >> f, "%d\t%d\t%s" % (
                labels[i], pred[i], " ".join(dataset[i][0])) # dataset include words (dataset[i][0]) and labels (dataset[i][1])
