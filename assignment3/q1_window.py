...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    Args:
        data: is a list of (sentence, labels) tuples. @sentence is a list
            ...
            output labels. Each word is itself a list of @n_features features.
            For example, the sentence "Chris Manning is amazing" and labels
            "PER PER O O" would become ([[1,9], [2,9], [3,8], [4,8]], [1, 1, 4, 4]).
            Here "Chris" the word has been featurized as "[1, 9]", and
            "[1, 1, 4, 4]" is the list of labels.
    ...
    for sentence, labels in data:
        extend_sentence = [start] * window_size + sentence + [end] * window_size
        l = 0
        for i in range(window_size, window_size + len(sentence)):
            temp_sent = []
            for j in range(i - window_size, i + window_size + 1):
                temp_sent.extend(extend_sentence[j])
            temp = (temp_sent, labels[l])
            l += 1
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        embedded = tf.Variable(self.pretrained_embeddings)  # (embed_size, embed_size)
        embeddings = tf.nn.embedding_lookup(embedded, self.input_placeholder)  # (batch_size, n_window_features, embed_size)
    ...
    def add_prediction_op(self):
        ...
        h = tf.nn.dropout(tf.nn.relu(tf.matmul(x, W) + b1), dropout_rate)
        ...
        b2 = tf.get_variable(name='b2',
                             ...
                             initializer=tf.constant_initializer(0))
        pred = tf.matmul(h, U) + b2  # we remain softmax op into loss calculation
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        """
        '''loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(self.labels_placeholder,
    ...
    def add_training_op(self, loss):
        ...
        lr = tf.train.exponential_decay(self.config.lr, global_step,
                                        self.config.ds, self.config.dr, staircase=True)
        train_op = tf.train.AdamOptimizer(learning_rate=lr).minimize(loss)
        return train_op

    def preprocess_sequence_data(self, examples):
        return make_windowed_data(examples, start=self.helper.START,
                                  end=self.helper.END, window_size=self.config.window_size)
