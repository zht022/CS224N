...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        add_length =  max_length - len(sentence)

        if add_length > 0:
            new_sentence = sentence + [zero_vector] * add_length
            new_labels = labels + [zero_label] * add_length
...
class RNNModel(NERModel):
    """
    Implements a recursive neural network with an embedding layer and single hidden layer.
    ...
    def add_embedding(self):
        ...
        """
        embeddings = tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings), self.input_placeholder)  # (batch_size, max_length, n_features, embed_size)
    ...
    def add_training_op(self, loss):
        ...
        lr = tf.train.exponential_decay(Config.lr, global_step,
                                        Config.ds, Config.dr, staircase=True)
