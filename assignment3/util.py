...
def minibatch(data, minibatch_idx):
    ''' In situations with labels, firstly read inputs, then read labels '''
    return data[minibatch_idx] if type(data) is np.ndarray else [data[i] for i in minibatch_idx]

def minibatches(data, batch_size, shuffle=True):
    ''' data is a list of tuple, each tuple has form of (list, label) '''
    batches = [np.array(col) for col in zip(*data)]  # zip(*data) returns [(input data lists), (labels)]; so batches = [input, labels]
